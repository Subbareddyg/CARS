...
public class UpdateAttributesFromPIMThread implements Runnable {
    ...
    private boolean processItemUpdate(CarsPimItemUpdate itemUpdate,
        ...
        if (carManager.isPostCutoverCar(car)) {
            if (itemType.equalsIgnoreCase(CarsPimItemUpdate.TYPE_STYLE_COLOR)
                        || itemType.equalsIgnoreCase(CarsPimItemUpdate.TYPE_PACK_COLOR)) {
                if(log.isInfoEnabled()){
                    log.info("UpdateAttributesFromPIMThread - postCutoverCar - STYLE_COLOR or PACK_COLOR - "
                            +" orin - " +orin);
                }
                updateImagesForCar(car);
                if(isPetUpdated && WorkflowStatus.CLOSED.equals(car.getCurrentWorkFlowStatus().getStatusCd())){
                    if (log.isInfoEnabled()){
                        log.info("CAR is closed and pet is updated  with STYLE_COLOR or PACK_COLOR " +" carId - "
                                +car.getCarId());
                    }
            ...
            if (WorkflowStatus.CLOSED.equals(car.getCurrentWorkFlowStatus().getStatusCd()) && !isPetUpdated) {
                if (log.isInfoEnabled()){
                ...
                try {
                    if (closedCar!=null) {
                        if (log.isInfoEnabled()){
                            log.info("car is closed and pet is not updated-UpdateAttributesFromPIMThread: closedCarId: "
                                    +closedCar.getCarId());
                        }
                ...
                if (itemType.equalsIgnoreCase(CarsPimItemUpdate.TYPE_STYLE)) {
                    if (log.isInfoEnabled()){
                        log.info("car is closed and pet is not updated-for TYPE_STYLE- UpdateAttributesFromPIMThread: "
                                +" carId : " +car.getCarId() +"- orin : "+orin);
                    }
                ...
                if (itemType.equalsIgnoreCase(CarsPimItemUpdate.TYPE_PACK)) {
                    if (log.isInfoEnabled()){
                        log.info("car is closed and pet is not updated - for TYPE_PACK- UpdateAttributesFromPIMThread:"
                                +" carId : " +car.getCarId() +"- orin : "+orin);
                    }
                ...
                if (itemType.equalsIgnoreCase(CarsPimItemUpdate.TYPE_STYLE)) {
                    if (log.isInfoEnabled()){
                        log.info("car is closed and pet is updated - for TYPE_STYLE - UpdateAttributesFromPIMThread: "
                                +" carId : " +car.getCarId() +"- orin : "+ orin);
                    }
                    boolean success = updateAttributesForStyleUpdate(itemUpdate.getOrin(), car);
                    if (!success) {
                ...
                if (itemType.equalsIgnoreCase(CarsPimItemUpdate.TYPE_PACK)) {
                    if (log.isInfoEnabled()){
                        log.info("car is closed and pet is updated - for TYPE_PACK - UpdateAttributesFromPIMThread: "
                                +" carId : " +car.getCarId() +"- orin : "+orin);
                    }
                    boolean success = updateAttributesForPackUpdate(createPackForItemUpdate(itemUpdate), car);
                    if (!success) {
                ...
                //reopening the car
                if (log.isInfoEnabled()){
                    log.info("car is closed and pet is updated - UpdateAttributesFromPIMThread: carId : "
                            +car.getCarId());
                }
                updateStatusForPostCutoverCar(car, itemUpdate);
                if(petDetails!=null && petUpdateDt!=null){
    ...
    private void updateStatusForPostCutoverCar(Car car, CarsPimItemUpdate itemUpdate) {
        ...
        if(log.isInfoEnabled()){
            log.info("updateStatusForPostCutoverCar - UpdaeAttributesFromPIMThread  - carId : " +car.getCarId());
        }
     ...
     * @param carIds
     */
    private void updateImagesForCar(Car car) {
        if (car != null) {
            try {
