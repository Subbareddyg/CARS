...
import com.belkinc.services.product.BelkProductServiceStub.UDAType0;
...
public class PIMAttributeManagerImpl extends UniversalManagerImpl implements
   ...
   @Override
                ...
                if(response!=null){
                    if(response.getErrorResponseData()!=null){
                        ...
                        for(ItemCatalogData catalog : itemCatList){
                            ...
                            //TODO: this below code shall be removed after load testing - start
                            log.info("sku map size in getAdditionalSkuDetailsFromSMART  "+skuDetailsMap.size());
                            for(Map.Entry<Long,List<AttributeData>> mapEntry : skuDetailsMap.entrySet()){
                                log.info("sku orin in getAdditionalSkuDetailsfromSMPART "+mapEntry.getKey());
                                List<AttributeData> skuList = mapEntry.getValue();
                                for(AttributeData skuData:skuList){
                                    log.info("Sku Attribute name "+skuData.getAttributeName());
                                    log.info("Sku Attribute value "+skuData.getAttributeValue());
                                }
                            }
                          //TODO: this above code shall be removed after load testing - end
    ...
    private Map<Long, List<AttributeData>> saveStyleColorAttributes(ItemCatalogData itemCatalogData,
        ...
        //TODO: below code needs to be removed after load test - start
        if(styleColorMap!=null && styleColorMap.size()>0){
            List<AttributeData> stylecolorAttr = styleColorMap.get(styleColorNum);

            for(AttributeData scAttr:stylecolorAttr){
                log.info("PIM Attribute Name " +scAttr.getAttributeName());
                log.info("PIM Attribute value " +scAttr.getAttributeValue());
            }
        }
        //TODO: below code needs to be removed after load test - end
        if(vs!=null && colorCd!=null){
            ...
            /*if(skuOrinMap!=null && skuOrinMap.size()>0){
                ...
                log.info("***** Inside saveStyleColorAttributes after uncommenting the sync method ********* ");
            }*/
        }else{
            if(log.isInfoEnabled()){
    ...
    public String encodeBrandWithSpecialCharacters(String strBrandAttrValue) {
        ...
        if (!hasInvalidChars) {
            strEncodedBrandAttrValue = StringEscapeUtils.escapeHtml4(strBrandAttrValue);
        }
