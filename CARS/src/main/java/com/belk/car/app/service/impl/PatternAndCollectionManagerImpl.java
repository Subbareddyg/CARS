...
public class PatternAndCollectionManagerImpl implements PatternAndCollectionManager {
     ...
     * @return
     */
    private Car mergePattern(VendorStyle parentVS, List<VendorStyle> childrenVS, User user) {
        if (log.isInfoEnabled()) {
        ...
        carCriteria.setVendorNumber(parentVS.getVendorNumber());
        carCriteria.setVendorStyleNumber(parentVS.getVendorStyleNumber());
        carCriteria.setStatusCd(Status.ACTIVE);
        carCriteria.setIgnoreUser(true);
        ...
        if (cars != null && !cars.isEmpty()) {
            //Choose one of the cars & set it as the Pattern Car
    ...
    private Map<String,List<VendorSku>> addSelectedColorSkusToSkuMap(Map<String,List<VendorSku>> skuMap, Car childCar, 
        ...
        try {
            ...
            if (Constants.STYLE_COLOR.equalsIgnoreCase(componentType)) {
                if (log.isInfoEnabled()) {
                    log.info("Child component is at the StyleColor level.  Therefore,"
                            + " add only the skus associated with StyleColor.");
                }
                ...
                if (log.isInfoEnabled()) {
                    log.info("Number of Skus added: " + skuList.size() + " / "
                            + skuSet.size() + " total.");
                }
            }
            else {
                if (log.isInfoEnabled()) {
                    log.info("Child component is at the Style level.  Therefore,"
                            + " add all skus associated with given Style.");
                }
                skuList.addAll(childCar.getVendorSkus());
            }
    ...
    private Object[] addChildrenToCollection(Car outfitCar, List<GroupingComponent> childrenToAdd, User user, boolean update) {
        ...
        addCollectionSkus(outfitProductCode, outfitChildProductsSku);
        outfitCar.setAuditInfo(user);
        carManager.saveOrUpdate(outfitCar);
    ...
    private void addChildrenToPattern(VendorStyle pattern, List<GroupingComponent> children, String vendorStyleType, User user, boolean update) {
        ...
        boolean eligibleForMerge = ((update && isExistingPatternEligibleForMerge(pattern)) 
                || (!update && isPatternEligibleForMerge(childrenToAdd)));
        // determine if its mergeable by the list of all children (including new ones)
        if (eligibleForMerge) {
            Car car = mergePattern(pattern, childrenToAdd, user); //returns the pattern car
            
            if (car!=null && Constants.FLAG_N.equals(car.getIsProductTypeRequired())) {
            ...
            List<Car> patternCars = convertPattern(pattern, childrenToAdd, user);
            updateAttributesAndImagesForCar(patternCars, defaultChild, vendorStyleType, false, user);
        }
    }
    ...
    /**
     * Check if pattern is eligible for "merging."
     * @param pattern
     * @return
     */
    public boolean isExistingPatternEligibleForMerge(VendorStyle pattern) {
        ...
        if (log.isInfoEnabled()) {
            log.info("Found " + patternCars + " associated with the parent pattern.");
        }
        if (patternCars == null || patternCars.size() > 1) {
            return false;
        } 
        else {
            if (patternCars.get(0).getContentStatus().getCode().contains("CMP")) {
                if (log.isInfoEnabled()) {
                    log.info("Pattern only has 1 car but already exists in CMP - need to convert.");
                }
                return false;
            }
        }
